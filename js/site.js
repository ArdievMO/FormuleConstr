 ...
 * @param {Object} rect - Объект данных блока.
 */
function rebuildBlockHeader(rect) {
    const result = resolveEquation(rect.formulaEq, rect.targetVar);
    const container = rect.element.querySelector('.formula-text');
    let newHtml = '';
    newHtml = `<div class="fText">${result}</div>`;
    container.innerHTML = newHtml;
}
...
function denormalizeExpression(expr) {
    expr = expr.replaceAll('**2', '^2');
    expr = expr.replaceAll('Math.pow(2,', '2^(');
    expr = expr.replaceAll('Math.cos', 'cos');
    expr = expr.replaceAll('cos', 'cos');
    expr = expr.replaceAll('Math.sin', 'sin');
    expr = expr.replaceAll('sin', 'sin');
    expr = expr.replaceAll('Math.PI', 'π');
    expr = expr.replaceAll('PI', 'π');
    expr = expr.replaceAll('Math.sqrt', '√');
...
async function buildGraph(startSpec, endSpec) {
    ...
    showGraphModal(graphContext);
}

/**
 * Генерирует массив точек для заданного диапазона X, используя контекст графика.
 */
function generatePoints(graphContext, xMin, xMax, pointsCount = 200) {
    const points = [];
...
/**
 * Показывает модальное окно с графиком.
 * @param {Object} graphContext - контекст (режим, функции, спецификации)
 */
function showGraphModal(graphContext) {
    // Удаляем предыдущее окно
    const existingModal = document.getElementById('graphModal');
    if (existingModal) existingModal.remove();

    // ---- Предварительная генерация точек (один раз) ----
    const POINTS_COUNT = 100;  // достаточно для плавного отображения
    const initialPoints = generateInitialPoints(graphContext, POINTS_COUNT);
    if (initialPoints.length === 0 || initialPoints.every(p => p.y === null)) {
        alert("Не удалось вычислить ни одной точки. Проверьте входные данные и связи.");
        return;
    }

    // ---- Состояние графика (масштаб) ----
    let state = {
        points: initialPoints,       // неизменный массив точек (x от -50 до 50)
        xMin: -50,
        xMax: 50,
        yMin: -50,
        yMax: 50
    ...
    header.style.cssText = `
        ...
        font-size: 1.2rem;
    `;
    header.innerHTML = `<span>График зависимости ${graphContext.yLabel} от ${graphContext.xLabel}</span>
                         <span style="cursor:pointer; font-size:28px;" id="closeGraphBtn">&times;</span>`;
    ...
    // ---- Функция перерисовки (использует текущий state) ----
    function redrawGraph() {
        drawCanvasGraphWithState(canvas, state, graphContext);
    }

    // ---- Функция обновления масштаба (не пересчитывает точки) ----
    function setZoom(newXMin, newXMax, newYMin, newYMax) {
        state.xMin = newXMin;
        state.xMax = newXMax;
        state.yMin = newYMin;
        state.yMax = newYMax;
    ...
    // ---- Обработчик колеса мыши (только масштабирование осей) ----
    function onWheel(e) {
        e.preventDefault();
        const delta = e.deltaY > 0 ? 1 : -1;   // -1: вверх (приближение), +1: вниз (отдаление)
        const factor = delta < 0 ? 0.9 : 1.1;  // при приближении умножаем диапазон на 0.9, при отдалении на 1.1

        // Центр текущей видимой области
        const xCenter = (state.xMin + state.xMax) / 2;
        const yCenter = (state.yMin + state.yMax) / 2;
...
function drawScaledGraph(canvas, state, graphContext) {
    ...
    const { points, scaleX, scaleY } = state;

    // Границы видимой области: исходный диапазон [-50,50] после масштабирования
    const xMin = -50 * scaleX;
    const xMax = 50 * scaleX;
    const yMin = -50 * scaleY;
    const yMax = 50 * scaleY;

    const pad = { left: 50, right: 30, top: 20, bottom: 40 };
    const graphW = width - pad.left - pad.right;
    const graphH = height - pad.top - pad.bottom;

    const toPixelX = (x) => pad.left + ((x - xMin) / (xMax - xMin)) * graphW;
    const toPixelY = (y) => pad.top + graphH - ((y - yMin) / (yMax - yMin)) * graphH;
    ...
    function niceStep(range, targetSteps = 10) {
        ...
        let residual = step / mag;
        let nice = (residual < 1.5) ? 1 * mag : (residual < 3.5) ? 2 * mag : (residual < 7.5) ? 5 * mag : 10 * mag;
        return nice;
    }
    ...
    for (let x = Math.ceil(xMin / xStep) * xStep; x <= xMax; x += xStep) {
        const px = toPixelX(x);
        if (px < pad.left || px > pad.left + graphW) continue;
        ctx.beginPath();
        ctx.moveTo(px, pad.top);
        ctx.lineTo(px, pad.top + graphH);
        ctx.stroke();
        ctx.fillText(x.toFixed(2), px - 10, pad.top + graphH + 15);
    }

    for (let y = Math.ceil(yMin / yStep) * yStep; y <= yMax; y += yStep) {
        const py = toPixelY(y);
        if (py < pad.top || py > pad.top + graphH) continue;
        ctx.beginPath();
        ctx.moveTo(pad.left, py);
        ctx.lineTo(pad.left + graphW, py);
        ctx.stroke();
        ctx.fillText(y.toFixed(2), pad.left - 30, py + 3);
    }
    ...
    if (xMin <= 0 && xMax >= 0) {
        ...
        ctx.lineTo(x0, pad.top + graphH);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(x0 - 5, pad.top);
        ctx.lineTo(x0 + 5, pad.top);
        ctx.lineTo(x0, pad.top - 8);
        ctx.fill();
    }
    if (yMin <= 0 && yMax >= 0) {
        const y0 = toPixelY(0);
        ctx.moveTo(pad.left, y0);
        ctx.lineTo(pad.left + graphW, y0);
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(pad.left + graphW, y0 - 5);
        ctx.lineTo(pad.left + graphW, y0 + 5);
        ctx.lineTo(pad.left + graphW - 8, y0);
        ctx.fill();
    }

    ctx.font = '12px "Segoe UI"';
    ctx.fillStyle = '#2c3e50';
    ctx.fillText(graphContext.xLabel, pad.left + graphW + 5, toPixelY(0) + 4);
    ctx.fillText(graphContext.yLabel, toPixelX(0) - 6, pad.top - 5);

    // ---- ГРАФИК: точки умножаются на scaleX, scaleY ----
    ctx.beginPath();
    ...
    for (let p of points) {
        ...
        const sx = p.x * scaleX;
        const sy = p.y * scaleY;
        const px = toPixelX(sx);
        const py = toPixelY(sy);
        if (px < pad.left || px > pad.left + graphW || py < pad.top || py > pad.top + graphH) {
            first = true;
            continue;
        }
        if (first) {
...
/**
 * Рисует график на canvas с учётом текущего состояния (масштаб).
 * @param {HTMLCanvasElement} canvas - элемент canvas
 * @param {Object} state - состояние { points, xMin, xMax, yMin, yMax }
 * @param {Object} graphContext - контекст для подписей осей
 */
function drawCanvasGraphWithState(canvas, state, graphContext) {
    const ctx = canvas.getContext('2d');
    const width = canvas.clientWidth;
    const height = canvas.clientHeight;
    canvas.width = width;
    ...
    // ---- Оси координат (если 0 в пределах диапазона) ----
    ctx.beginPath();
    ctx.strokeStyle = '#333';
    ctx.lineWidth = 2;
    if (xMin <= 0 && xMax >= 0) {
    ...
    // ---- Рисование графика (просто соединяем точки, обрывая линию при отсутствии y) ----
    ctx.beginPath();
    ctx.strokeStyle = '#f97316';
    ctx.lineWidth = 2;
    let firstPoint = true;
    for (let p of points) {
        ...
        // Отсекаем точки, выходящие за пределы видимой области по X или Y (чтобы линии не уходили за край)
        if (px < padding.left || px > padding.left + graphWidth || py < padding.top || py > padding.top + graphHeight) {
            firstPoint = true;
            continue;
        }
        if (firstPoint) {
...
function getInputExpression(rect, paramName, sourceRect, sourceParam, visited) {
    ...
    // Ищем связь, которая подаёт значение на этот вход
    const conn = paramConnections.find(c => c.targetRectId === rect.id && c.targetParam === paramName);
    if (conn) {
        const srcRect = rectangles.get(conn.sourceRectId);
        if (!srcRect) return null;
        // Выходной параметр исходного блока – это его targetVar
        if (srcRect.targetVar !== conn.sourceParam) return null; // должно совпадать
        const srcExpr = getOutputExpression(srcRect, sourceRect, sourceParam, visited);
        visited.delete(key);
        return srcExpr;
    } else {
...
/**
 * Генерирует массив точек для заданного X-диапазона.
 * Если передан параметр 'fullRange=true', то вычисляются все точки от -50 до 50.
 * @param {Object} graphContext - контекст графика (mode, analyticFn, startSpec, endSpec)
 * @param {number} xMin - левая граница X
 * @param {number} xMax - правая граница X
 * @param {number} pointsCount - количество точек
 * @param {boolean} fullRange - если true, вычисляет для всего диапазона [-50,50] (игнорируя xMin, xMax)
 * @returns {Array} массив точек {x, y}
 */
function generatePoints(graphContext, xMin, xMax, pointsCount, fullRange = false) {
    let startX, endX;
    if (fullRange) {
        startX = -50;
        endX = 50;
    ...
    if (graphContext.mode === 'analytic' && graphContext.analyticFn) {
        for (let i = 0; i <= pointsCount; i++) {
            const x = startX + (i / pointsCount) * (endX - startX);
            let y;
            try {
                y = graphContext.analyticFn(x);
            } catch (e) {
...
/**
 * Фильтрует массив точек по X-диапазону (с сохранением порядка).
 * @param {Array} points - исходный массив точек
 * @param {number} xMin - левая граница X
 * @param {number} xMax - правая граница X
 * @returns {Array} отфильтрованный массив точек (те, у которых x между xMin и xMax)
 */
function filterPointsByX(points, xMin, xMax) {
