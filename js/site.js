...
function showHelp() {
    ...
    let content = `
        <p><strong>Конструктор формул</strong></p>
        ...
        <ul>
            ...
            <li>Карточка содержит:
                <ul>
                    ...
                    <li>синие кружки – INput параметры (можно задать число или связать с другим блоком);</li>
                    <li>оранжевый кружок – OUTput параметр (вычисляемый);</li>
        ...
        <ul>
            <li><strong>Перетаскивание:</strong> зажмите левую кнопку на заголовке блока.</li>
            <li><strong>Удаление:</strong> нажмите крестик (❌) в правом верхнем углу блока.</li>
        </ul>

        <h3>3. Связывание параметров (передача данных)</h3>
        <ul>
            <li>Нажмите кнопку <strong>«Режим связи (OFF)»</strong>, после чего состояние изменится на <strong>(ON)</strong>.</li>
            ...
            <li>Между блоками появится пунктирная линия → значение из OUTput параметра первого блока будет автоматически передаваться в INput второго.</li>
            <li><strong>Важно:</strong> INput параметр может быть связан только с одним OUTput. Если параметр уже связан, удалите связь в нижней панели.</li>
            <li>Все активные связи отображаются внизу под графической областью – их можно удалять по отдельности или все сразу кнопкой «Удалить все».</li>
            <li>Для выхода из режима связи нажмите кнопку ещё раз (станет OFF).</li>
        </ul>

        <h3>4. Ввод чисел</h3>
        <ul>
            <li>У каждого INput параметра есть поле ввода. Введите число (разделитель – точка или запятая).</li>
            <li>Если параметр связан с OUTput другого блока, то поле ввода становится недоступным для ручного изменения и отображает переданное значение.</li>
        </ul>

        <h3>5. Смена целевой переменной (OUTput параметр)</h3>
        <ul>
            <li>По умолчанию блок решает уравнение относительно первого параметра (например, <code>F = m*a</code> вычисляет <code>F</code>).</li>
            <li><strong>Кликните по серой области</strong> с названием OUTput параметра – целевая переменная циклически переключится на следующий параметр формулы.</li>
            <li>Поля ввода для остальных переменных в блоке автоматически обновятся.</li>
            <li>P.S. Переключение целевой переменной недоступно:
                <ul>
                    <li>для блоков, соединенных в связи</li>
                    <li>для блоков из раздела "Дополнительно"</li>
                </ul>
            </li>
        </ul>
        ...
        <ul>
            ...
            <li>Вычисленное значение отображается рядом с OUTput параметром (оранжевый кружок).</li>
            <li>Если данных недостаточно, решение невозможно или решений нет, то отображается: «?».</li>
        </ul>
        ...
        <ul>
            ...
            <li><strong>Затем кликните на оранжевый кружок</strong> – это будет зависимая переменная (Y).</li>
            <li>С небольшой задержкой построится график <strong>Y = f(X)</strong>.</li>
            <li>В блоке графика доступны:
                <ul>
                    <li><strong>Панорамирование:</strong> зажмите левую кнопку мыши (ЛКМ) и двигайте.</li>
                    <li><strong>Масштабирование:</strong> крутите колёсико мыши.</li>
                    <li><strong>Смена зависимости:</strong> нажмите кнопку "🔄 инверсия" – построится график обратной зависимости: <strong>X = g(Y)</strong>, если это возможно.</li>
                    <li><strong>Удаление:</strong> нажмите крестик (❌) в правом верхнем углу блока.</li>
                </ul>
            </li>
            <li>P.S. Построение графика обратной зависимости невозможно при наличии параллельного соединения между блоками с независимой и зависимой переменными.</li>
        </ul>

        <h3>8. Сохранение и загрузка проекта</h3>
        <ul>
            <li><strong>«СОХРАНИТЬ»:</strong> скачивает JSON‑файл со всеми блоками формул, связями, графиками.</li>
        ...
        <ul>
            <li>Нажмите кнопку <strong>«ДЕМО»</strong> – откроется список с демонстрационными вариантами использования программы.</li>
            <li>Выберите любой пункт – программа загрузит соответствующие блоки, связи и графики.</li>
        </ul>

        <h3>10. Графическая область</h3>
        <li>Изначально область определяется фиксированного размера, но при приближении блоков к правой и нижней границам области, область постепенно расширяется в соответствующую сторону</li>
        <ul>
            <li><strong>Панорамирование:</strong> зажмите правую кнопку мыши (ПКМ) и двигайте.</li>
            <li><strong>Масштабирование:</strong> крутите колёсико мыши на графической области.</li>
        </ul>

        <h3>11. Советы и ограничения</h3>
        <ul>
            <li>Выбор ветки (переключатель в блоке) для квадратных уравнений влияет на вычисления только при установке в качестве OUTput параметра - корня квадратного уравнения.</li>
            <li>При переключении зависимости блоки пересчитываются, поэтому график может изменится при изменении значения какого-либо параметра.</li>
            <li>Не рекомендуется построение графиков при выборе переменных из блоков, соединенных параллельно.</li>
        </ul>
    `;
