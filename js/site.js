...
function handlePortClick(rectId, param, isOutput) {
    ...
    if (graphModeActive) {
        if (selectingStart) {
            // Выбираем начальный параметр – только входной (синий, isOutput === false)
            if (isOutput) {
                alert("Начальный параметр должен быть ВХОДНЫМ (синий кружок). Выберите другой параметр.");
                return;
            }
            graphStartParam = { rectId, paramName: param, isOutput: false };
            selectingStart = false;
            selectingEnd = true;
            modeStatusSpan.innerText = 'Теперь выберите КОНЕЧНЫЙ параметр (любой кружок)';
...
function buildGraph(startSpec, endSpec) {
    // Проверка существования блоков
    if (startSpec.rectId === null) {
        alert("Ошибка: блок начального параметра не найден");
        return;
    }
    ...
    if (!endRect) {
        alert("Ошибка: блок конечного параметра не найден");
        return;
    }

    // Проверка, что параметры существуют в блоках
    ...
    // ========== ФИКСИРОВАННЫЙ ДИАПАЗОН X ОТ -50 ДО +50 ==========
    const xMin = -50;
    const xMax = 50;
    const pointsCount = 100;   // количество точек (больше = глаже)

    // Генерируем массив значений X
    const xValues = [];
    ...
    // Сохраняем исходное состояние начального параметра (ручное значение и связь)
    ...
    // Временно отключаем перерисовку линий и списка связей (для производительности)
    ...
    try {
        for (let x of xValues) {
            // Удаляем временную связь, если она была (чтобы установить ручное значение)
            if (originalConnection) {
                paramConnections.splice(originalConnectionIndex, 1);
            }
            // Устанавливаем текущее значение X как ручной ввод
            startRect.manualValues[startSpec.paramName] = String(x);
            rebuildParamsList(startRect);

            // Пересчитываем все блоки с новым значением
            computeAll();

            // Получаем значение конечного параметра (Y)
            let y = null;
            if (endSpec.isOutput) {
                ...
                if (endConn) {
                    ...
                    if (manualVal !== undefined && manualVal !== '' && !isNaN(Number.parseFloat(manualVal))) {
                        y = Number.parseFloat(manualVal);
                    }
                }
            }
            ...
            if (y !== null && isFinite(y)) {
                ...
                points.push({ x, y: null });   // точка не определена
            }
        }
    } finally {
        // Восстанавливаем исходное состояние начального параметра
        if (originalConnection) {
            // Возвращаем связь
            ...
            // Удаляем временное ручное значение, если его не было
        ...
        // Восстанавливаем функции отрисовки
    ...
    // Фильтруем точки, где Y определён (хотя бы одна)
    ...
    // Отображаем график в модальном окне
    showGraphModal(validPoints, startSpec.paramName, endSpec.paramName);
}

// Функция отображения графика в модальном окне с canvas
function showGraphModal(points, xLabel, yLabel) {
    ...
    header.style.cssText = `
        ...
        font-size: 1.2rem;
    `;
    header.innerHTML = `<span>График зависимости ${yLabel} от ${xLabel}</span>
    ...
    // Рисуем график после того, как canvas появится в DOM
    setTimeout(() => {
        drawCanvasGraph(canvas, points, xLabel, yLabel);
    }, 50);
...
function drawCanvasGraph(canvas, points, xLabel, yLabel) {
    ...
    const yToPixel = (y) => padding.top + graphHeight - ((y - minY) / (maxY - minY)) * graphHeight;
    ...
    for (let x = -50; x <= 50; x += 10) {
        ...
        ctx.stroke();
        ctx.fillText(x.toString(), px - 10, padding.top + graphHeight + 15);
    }
    ...
    for (let y = -50; y <= 50; y += 10) {
        ...
        ctx.stroke();
        ctx.fillText(y.toString(), padding.left - 30, py + 3);
    }
    ...
    // Ось X – подпись справа от стрелки
    ctx.fillText(xLabel, padding.left + graphWidth + 5, yZero + 4);
    // Ось Y – подпись сверху от стрелки
    ctx.fillText(yLabel, xZero - 6, padding.top - 5);

    // Рисуем график (без обрезания – точки за пределами [-50,50] пропускаем)
    ctx.beginPath();
    ...
    for (let p of points) {
        // Проверяем, находится ли Y в допустимом диапазоне
        if (p.y === null || p.y < minY || p.y > maxY) {
            first = true;   // разрываем линию
            continue;
        }
