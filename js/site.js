...
function getInversePath(startSpec, endSpec) {
    ...
    for (let i = 0; i < pathKeys.length; i++) {
        const [rectId, param] = pathKeys[i].split(':');
...
async function buildGraph(startSpec, endSpec) {
    ...
    blockDiv.innerHTML = `
        <div class="card-header" style="cursor:move;">
            ...
            <div style="display:flex; gap:5px;">
                <button class="swap-dep-btn" style="background:none; border:none; cursor:pointer; font-size:16px;">🔄</button>
                <div class="delete-card">✕</div>
            </div>
        </div>
    ...
    if (swapBtn) {
        swapBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            const newStart = { rectId: endSpec.rectId, paramName: endSpec.paramName, isOutput: false/*endSpec.isOutput*/ };
            const newEnd   = { rectId: startSpec.rectId, paramName: startSpec.paramName, isOutput: true/*startSpec.isOutput*/ };
            ...
            buildInverseGraph(newStart, newEnd, targetMap);
        });
    }
...
async function buildInverseGraph(startSpec, endSpec, targetMap) {
    const preparation = prepareGraphEvaluation(startSpec, endSpec);
    if (!preparation.canCompute) {
        alert(preparation.errorMessage);
        return;
    ...
    if (swapBtn) {
        swapBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            const newStart = { rectId: endSpec.rectId, paramName: endSpec.paramName, isOutput: false/*endSpec.isOutput*/ };
            const newEnd   = { rectId: startSpec.rectId, paramName: startSpec.paramName, isOutput: false/*startSpec.isOutput*/ };
            deleteGraphBlock(blockId);
            buildGraph(newStart, newEnd);
        });
    }
